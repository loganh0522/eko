...
describe Business::ClientContactsController do 
  ...
  before do  
    ...
    job_board
    @client_contact = Fabricate(:client_contact, client: client) 
  ...
  describe "GET show" do
    it_behaves_like "requires sign in" do
      let(:action) {get :show, id: @client_contact.id}
    end
    ...
    context "client_contact belongs to the company" do 
      before do 
        xhr :get, :show, id: @client_contact.id
      end

      it "sets client_contact to the instance of client_contact" do 
        expect(assigns(:client_contact)).to eq(@client_contact)
      end
    end
  end
  ...
  describe "GET new" do 
    ...
    before do  
      xhr :get, :new, cliend_id: client.id
    end

    it "sets @client_contact to be a new instance of client_contact" do 
      expect(assigns(:client_contact)).to be_new_record 
      expect(assigns(:client_contact)).to be_instance_of ClientContact 
    end
  ...
  #     it "only renders client_contacts that belong to company" do
  #       expect(company.client_contacts.count).to eq(2)
  #       expect(assigns(:client_contacts)).to eq([@client_contact, ClientContact.last])
  #     end
  ...
  #       expect(ClientContact.first.company_name).to eq("Thomas")
  #     end
